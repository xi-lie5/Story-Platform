...
router.get('/me/stats', authGuard, cacheMiddleware(60), async (req, res, next) => {
  try {
    ...
    // 统计创作中的故事数量（所有非已发布状态的故事：draft, pending, rejected等）
    // 使用 NOT IN 查询，排除 'published' 状态，确保逻辑正确
    const draftStories = await Story.countDocuments({ 
      author_id: userIdInt,
      status: ['draft', 'pending', 'rejected', 'unpublished']
    });
    
    // 添加调试日志
    console.log(`[STATS] 用户 ${userIdInt} 统计: 已发布=${publishedStories}, 创作中=${draftStories}`);
    
    // 统计收藏的故事数量
    const collectedStories = await Collection.countByUser(userId);
    
    // 统计总故事数量
    const totalStories = await Story.countDocuments({ author_id: userIdInt });
    
    // 获取最近的故事统计（最近30天）
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    
    // 注意：这里需要根据实际的Story模型调整查询方式
    // 如果Story模型支持日期范围查询，需要修改查询条件
