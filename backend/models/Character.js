...
class Character {
  ...
  /**
   * 保存角色（实例方法）
   * @returns {Promise<Object>} 保存后的角色对象
   */
  async save() {
    if (this.id) {
      // 更新现有角色
      return await Character.findByIdAndUpdate(this.id, {
        name: this.name,
      ...
      const created = await Character.create({
        storyId: this.storyId || this.story_id,
        name: this.name,
        description: this.description
      });
      this.id = created.id;
      this.storyId = created.story_id;
      this.story_id = created.story_id;
   ...
   * @returns {Promise<Character>} 创建的角色实例
   */
  static async create(characterData) {
    const connection = await pool.getConnection();
    try {
      const { storyId, story_id, name, description } = characterData;
      const storyIdValue = storyId || story_id;
      
      // 验证必填字段
      if (!storyIdValue || !name || !description) {
      ...
      await connection.execute(
        `INSERT INTO characters (id, story_id, name, description) 
         VALUES (?, ?, ?, ?)`,
        [characterId, storyIdValue, name.trim(), description.trim()]
      );
   ...
   * @returns {Promise<Character|null>} 角色实例或null
   */
  static async findById(id) {
    ...
    try {
      ...
      return characters[0] ? new Character(characters[0]) : null;
    } finally {
      connection.release();
    }
  }
